...
export default function App() {
  ...
  useEffect(() => {
    // Safety Valve: Stop spinner after 8 seconds if Firebase hangs
  ...
  const handleLogin = async () => {
    ...
    try {
      if (Capacitor.isNativePlatform()) {
        // 1. Trigger Native iOS Google Sign-In
        const result = await FirebaseAuthentication.signInWithGoogle({
          scopes: ['email', 'profile'],
          skipNativeAuth: false, 
        });

        // 2. MANUAL BRIDGE: Hand the native token to the Web SDK
      ...
      setIsLoggingIn(false);
      // Don't show error if user just swiped the popup away
